...
chrome.runtime.onConnect.addListener(function(port) {
  ...
  port.onMessage.addListener(function(msg) {
    ...
    if (msg.msg === "Abracadabra!") {
      startURL = window.location.search.split('startURL=')[1] ? window.location.search.split('startURL=')[1] : '';
      startURL = decodeURIComponent(startURL);
      console.log("startURL: "+startURL);
      port.postMessage({msg: "Alibaba", domain: document.domain.toString().toLowerCase(), starturl: startURL });
    } else if (msg.msg === "FORM"){
	var filled = 0;
	document.onmousedown = function(evt) {
	    //alert("clicked");
		...
		//var keyEvent = document.createEvent("KeyboardEvent");
        	...
        	//elements[i].dispatchEvent(keyEvent);
